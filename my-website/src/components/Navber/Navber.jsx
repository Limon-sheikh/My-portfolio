...
const Navbar = () => {
  ...
  return (
    <>
      ...
      {/* Mobile Menu Drawer */}
      <div className={`fixed top-0 right-0 h-full w-[60vw] shadow-xl z-[70] border-l border-white/20 backdrop-blur-2xl bg-white/10 md:hidden transform transition-transform duration-300 ease-in-out ${open ? "translate-x-0" : "translate-x-full"}`}>
        <nav className="mt-20">
      ...
      {/* Backdrop Layer */}
      {open && (<div onClick={toggle} className="fixed inset-0 md:hidden bg-black/30 backdrop-blur-xl z-[60] transition-opacity duration-300"></div>)}
    </>
  );
};
